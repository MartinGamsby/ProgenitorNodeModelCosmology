...
The model makes a quantitative, falsifiable prediction: the virialized nature of the meta-structure produces a dipole anisotropy in the Hubble constant of $\Delta H_0/H_0 \approx 4$-$11\%$, comparable to the observed Hubble Tension ($\sim$8.6\%) \cite{Riess2021}.

While this work presents a simplified toy model that does not yet address early universe physics or CMB observations, it demonstrates that dark energy is not inevitable;
classical gravitational alternatives exist. This proof-of-concept challenges the assumption that the cosmological principle holds at super-horizon scales and demonstrates that classical gravitational alternatives to dark energy are viable.
\end{abstract}
...
Critically, because all multipoles originate from the \textbf{same underlying lattice geometry}, their preferred axes are necessarily correlated---the quadrupole and octopole planes share the principal axes of the HMEA grid.
This is precisely the alignment observed in the CMB data.

\paragraph{Connection to the Progenitor Hypothesis:}
If the Big Bang originated from a destabilized Progenitor Node (Section 2.2), the expanding matter would inherit the tidal imprint of the surrounding lattice from the earliest moments.
The preferred direction of the Axis of Evil would correspond to the direction of the nearest or most massive HMEA node---the same direction that produces the predicted $H_0$ dipole (Section 7.2).
The ecliptic correlation, while potentially coincidental, could reflect the cumulative gravitational torque from the nearest node influencing the formation and orientation of the Solar System's angular momentum over cosmic time.
...
The dark flow direction ($l \approx 290°$, $b \approx 30°$ from Kashlinsky; $l \approx 298°$, $b \approx -8°$ from Watkins) is in the same general region of the sky as the CMB Axis of Evil ($l \approx 260°$, $b \approx 60°$).
In the External-Node Model, all three anomalies---the $H_0$ dipole, the CMB multipole alignment, and the dark flow---should point toward the nearest or most massive HMEA node.
The approximate directional concordance across independent measurements and independent methods (kSZ, distance indicators, CMB multipoles) is a natural consequence of a single underlying lattice geometry.
Precise alignment is not expected because the real meta-structure is irregular rather than a perfect cubic lattice.
...
While this work presents a simplified toy model with acknowledged limitations, particularly regarding early universe physics, CMB observations, and the need for full relativistic treatment, it demonstrates several important principles.
...
Third, \textbf{the mechanism is viable}: external structure could, in principle, explain cosmic acceleration.
...
The model provides natural explanations for the timing of acceleration (relating it to geometric scales rather than arbitrary constants) and may offer alternative interpretations of current anomalies such as the Hubble Tension, the CMB Axis of Evil (large-angle multipole alignments), dark flow (large-scale bulk motions), and the population of intermediate-mass black holes.
Future work must focus on extending the framework to address early universe physics, testing against CMB and large-scale structure data, developing the full general relativistic formulation, and quantifying predictions for anisotropy measurements in upcoming surveys (Euclid, LSST).
While the existence of trans-observable structure remains speculative, the remarkable agreement between this toy model and observations suggests that alternatives to dark energy deserve serious theoretical investigation.
