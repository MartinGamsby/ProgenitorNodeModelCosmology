...
N-body simulations over the late-universe era ($t=5.8 \rightarrow 13.8$ Gyr) reproduce $\Lambda$CDM expansion with \textbf{$R^2 > 0.95$ for expansion rate} and \textbf{$R^2 > 0.99$ for size}, using configurations spanning $M = 92$-$9000 \times M_{obs}$, $S = 15$-$38$ Gpc. Compared directly against the Pantheon$+$SH0ES supernovae, the from-simulation distance--redshift relation fits at \textbf{$\chi^2/\mathrm{dof} \approx 0.44$} --- statistically indistinguishable from $\Lambda$CDM ($0.436$) and decisively rejecting the no-dark-energy Einstein--de~Sitter null ($0.843$) --- and does so for \emph{many} distinct configurations, a match we confirm to be numerically converged up to $10^5$ particles.

The same lattice geometry, with no additional parameters, also produces patterns consistent with three large-scale anomalies: the Hubble Tension ($\Delta H_0/H_0 \approx 5$-$11\%$ \cite{Riess2022}), the CMB ``Axis of Evil'' \cite{Land2005}, and dark flow \cite{Watkins2023}. The model additionally predicts a distinctive phantom-like hyper-acceleration ($w < -1$) as the bubble approaches the surrounding nodes (Section~\ref{sec:phantom}).
We do not attempt to model early-universe dynamics or provide a complete theoretical resolution to these large-scale anomalies; rather, our simulation provides a proof-of-concept that classical gravity is sufficient to reproduce late-time acceleration, implying that tidal forces alone might account for the dynamics we currently label as dark energy.
\end{abstract}
...
Together, the External-Node mechanism and the Progenitor Hypothesis constitute what we term the \textbf{Progenitor Node Model}.
...
Figure~\ref{fig:meta-structure} shows this local arrangement: the HMEA nodes in our neighbourhood, sized and coloured by mass, with our observable universe drawn to scale. Because a node's tidal pull falls as $1/d^3$ with distance (Section~\ref{sec:tidal}), the nearby nodes dominate the dynamics; the meta-structure's mass segregation (more massive nodes further out, as in a relaxed cluster) must therefore stay mild enough that these near nodes retain masses of order $M_{ext}$.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\linewidth]{fig_meta_structure.png}
    \caption{The virialized meta-structure in our neighbourhood. The HMEA nodes within $50$ Gpc ($2.5\times S$) of the observer, from a force-balanced virialized lattice ($300$ nodes, log-normal mass function with spread $\sigma=1$ and mass segregation, $S=20$ Gpc), sized and coloured by mass. Our observable universe (blue sphere, $R\approx14$ Gpc) sits at the central site, just inside the first node shell at $d=S=20$ Gpc. More massive nodes lie further out (mass segregation); because a node's tidal pull falls as $1/d^3$, the nearby nodes dominate the expansion, so the physically relevant configurations keep them at masses of order $M_{ext}$.}
...
The critical result here is the linear $R$ dependence. In the standard Friedmann equations, the cosmological constant drives an acceleration of $H_0^2 \Omega_\Lambda R$. Consequently, in the non-relativistic limit, this single-axis external tidal field is mathematically indistinguishable from $\Lambda$ \emph{along the axis to a node}. The following subsection sharpens this statement: the \emph{full} three-dimensional symmetric field is traceless, so the indistinguishability holds for the axial magnitude but not for the isotropic volume term.

\subsection{Traceless Tides: a Correction to the Isotropy Claim}
\label{sec:traceless}
The single-axis derivation above (Equation~\ref{eq:tidal-linear}) is correct as the radial acceleration \emph{toward the nearest node}, but it must not be read as an isotropic, $\Lambda$-like volume term. In the vacuum between the nodes the external potential satisfies Laplace's equation, $\nabla^2 \Phi_{ext} = \sum_{\text{nodes}} \nabla^2(-GM_{ext}/r) = 0$ away from the sources. Therefore the external tidal tensor $T_{ij} = -\partial_i \partial_j \Phi_{ext}$ is \textbf{traceless}: $\mathrm{Tr}\,T = -\nabla^2 \Phi_{ext} = 0$. A traceless tensor cannot drive net isotropic expansion to linear order in $R/S$; it can only produce \textbf{shear}.

Concretely, for the symmetric equal-mass lattice the tidal tensor at the exact centre \emph{vanishes} by symmetry. Making one face heavier (or one axis tighter) yields a tensor of the form
\begin{equation}
T \;\sim\; \frac{G M_{ext}}{S^3}\,\mathrm{diag}(+4,\,-2,\,-2),
\label{eq:traceless-shear}
\end{equation}
which is manifestly traceless: a pure stretch along one axis with compensating compression along the other two, plus a net dipole. There is no $\mathrm{diag}(+1,+1,+1)$ isotropic piece available at this order.

The model's match to the \emph{isotropic} root-mean-square expansion history is therefore \textbf{not} the linear tidal term. It arises from two genuinely present but distinct effects: (i) the \textbf{nonlinear} $(S-R)^{-2}$ growth of the force as particles move off-centre toward the nodes (Equation~\ref{eq:tidal-exact}, beyond the linear Taylor term), and (ii) the calibrated initial Hubble-flow velocities (Section~\ref{sec:framework}). A direct consequence, confirmed numerically (Section~\ref{sec:from-data}), is that the isotropic supernova Hubble diagram \emph{cannot} discriminate the lattice configuration: the reduced $\chi^2$ is essentially flat in $M$ once each $M$ is paired with its growth-anchored $S$, because the data constrain only the effective expansion ($\sim$\,effective $\Omega_\Lambda$), i.e. the ratio $M/S^3$. This is the quantitative footprint of the $M/S^3$ degeneracy noted in Section~\ref{sec:results}.

This correction \emph{sharpens} the model rather than retracting it. The isotropic background is reproduced, but the model's real, distinguishing signal lives in the \textbf{anisotropy} it predicts---a shear and an associated Hubble dipole---which the per-node HMEA masses directly control (Sections~\ref{sec:dipole}--\ref{sec:dark-flow}). This is exactly the regime the standard $\Lambda$ term cannot mimic, and it is where the model is genuinely falsifiable.
...
\subsection{Comparison to Real Supernova Data}
\label{sec:from-data}

The $R^2$ results above compare the simulated expansion to the \emph{analytic} $\Lambda$CDM curve---a model-versus-theory test. As a stronger, non-circular check we now compare the simulation directly to \textbf{real data}: the Pantheon$+$SH0ES Type~Ia supernova compilation \cite{Brout2022}.

\paragraph{Method (non-circular).}
We run the External-Node dynamics from $t = 2.9$ Gyr to today (covering the observed range $z \lesssim 2.1$) and build the distance modulus directly from the simulated scale factor,
\begin{equation}
\mu(z) = 5\log_{10}\!\left[\frac{(1+z)\,c\!\int_{t}^{t_0}\! dt'/a(t')}{10\,\mathrm{pc}}\right],
\label{eq:mu-from-sim}
\end{equation}
integrating the \emph{actual} $a(t)$ the particles produce (internal gravity, the tidal nodes, Hubble drag). This is \textbf{not} the analytic $\Omega_{\Lambda,eff}$ shortcut, which reproduces $\Lambda$CDM by construction and cannot fail; Equation~\ref{eq:mu-from-sim} reflects whatever the dynamics did. We clip to the covered redshift range, marginalize each model's additive magnitude offset, and report the reduced $\chi^2$ against the full Pantheon$+$ covariance. Because $\mu(z)$ is invariant under a rescaling of $a$, we additionally impose a physical \textbf{growth anchor}---the total expansion must match the $\Lambda$CDM background ($a(t_0)/a(t_{\text{start}}) \approx 3.30$ at $t_{\text{start}} = 2.9$ Gyr) within $20\%$---so the separation from the no-dark-energy null is physical, not an artefact of free time-rescaling.

\paragraph{Result.}
The from-simulation fit reaches $\chi^2/\mathrm{dof} \approx 0.44$---statistically indistinguishable from analytic $\Lambda$CDM ($0.437$)---and sits far from the Einstein--de~Sitter no-dark-energy null ($0.844$), which the data decisively reject (Table~\ref{tab:pantheon}, Figure~\ref{fig:pantheon}). This headline uses the model's realistic form: a force-balanced virialized node lattice with a wide mass function, scored at the \textbf{best-matching observer} (we are not at the cloud's centre). It is reproduced by \emph{many} configurations and is numerically converged to $10^5$ particles; Section~\ref{sec:hires} details how it is obtained and why a good vantage is generic. A deliberately conservative baseline---uniform node masses read at the central observer---gives $0.501$, still \emph{with} $\Lambda$CDM and far from the null.

\begin{table}[htbp]
\centering
\begin{tabular}{lc}
\toprule
\textbf{Model} & \textbf{$\chi^2/\mathrm{dof}$} \\
\midrule
External-Node --- virialized, best observer (headline) & $\approx 0.44$ \\
External-Node --- uniform nodes, central observer (baseline) & $0.501$ \\
$\Lambda$CDM (analytic)                & $0.437$ \\
Einstein--de~Sitter null ($\Omega_m=1$) & $0.844$ \\
\bottomrule
\end{tabular}
\caption{Offset-marginalized fits of the from-simulation External-Node $\mu(z)$ to real Pantheon$+$SH0ES data (over the simulation-covered range, $\sim$1300--1560 SNe depending on the observer), versus analytic $\Lambda$CDM and the Einstein--de~Sitter no-dark-energy null. The headline uses the virialized structure at the best-matching observer (Section~\ref{sec:hires}); the baseline uses uniform nodes at the centre. The reduced $\chi^2 < 1$ reflects the conservative Pantheon$+$ covariance; the informative quantities are the \emph{separations}, not the absolute fit quality.}
\label{tab:pantheon}
\end{table}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\linewidth]{fig4_pantheon.png}
    \caption{From-simulation External-Node $\mu(z)$ at the best-matching observer (orange) compared to Pantheon$+$SH0ES data (grey), analytic $\Lambda$CDM (blue dashed), and the Einstein--de~Sitter no-dark-energy null (green). Lower panel: residuals relative to $\Lambda$CDM. The simulated model tracks $\Lambda$CDM and the data ($\chi^2/\mathrm{dof} \approx 0.44$) and is far from the Einstein--de~Sitter null ($0.85$), which the data rule out. (Headline configuration $M=300\,M_{obs}$, $S=20$ Gpc, virialized lattice, $10^5$ particles; see Section~\ref{sec:hires}.)}
\label{fig:pantheon}
\end{figure}

\paragraph{Interpretation (stated plainly).}
Three honest conclusions follow. \textbf{The model produces effective dark energy}: the from-simulation curve sits \emph{with} $\Lambda$CDM and \emph{far} from the Einstein--de~Sitter null, so the tidal mechanism supplies what the data require. \textbf{It matches but does not beat $\Lambda$CDM}: at $\approx 0.44$ versus $0.437$ the two are indistinguishable at observable redshifts. \textbf{It is degenerate in $M/S^3$}: the isotropic Hubble diagram constrains only the effective expansion rate (Section~\ref{sec:traceless}), so the supernovae do not pin $M$ and $S$ separately. We do not read $\chi^2/\mathrm{dof} < 1$ as an exceptional fit---it reflects the conservative Pantheon$+$ covariance; the informative quantities are the \emph{separations}. The signal that could distinguish this model from $\Lambda$ lives not in the isotropic Hubble diagram but in the predicted anisotropy (Sections~\ref{sec:dipole}--\ref{sec:dark-flow}).

\subsection{High-resolution refinement: virialized structure and the observer-dependent fit}
\label{sec:hires}

The $0.501$ result above uses a \emph{uniform}-mass node shell and the cloud's central observer. Two physically-motivated refinements sharpen it. First, we replace the uniform shell with a force-balanced \textbf{virialized lattice carrying a wide node-mass function} (a log-normal mass distribution, mass-segregated so the most massive nodes sit furthest out), modelling a relaxed meta-structure rather than equal-mass points; the wide mass function avoids the small-$S$ cloud collapse and reaches the genuinely accelerating regime. Second, and more importantly, we drop the assumption that we sit at the cloud's centre. The probability of occupying the exact centre is negligible, and the model predicts a \emph{position-dependent} expansion history; the supernova data localise us to whichever vantage they match. We therefore treat every particle as a candidate observer (its $\mu(z)$ built from the local growth of its own neighbourhood) and report the \textbf{best-matching observer}, alongside the \emph{fraction} of vantages below each reference so the genericity of a good vantage stays visible.

With these refinements the from-simulation fit improves to $\chi^2/\mathrm{dof} \approx 0.44$ --- essentially $\Lambda$CDM ($0.436$) --- and, crucially, this is reached by \textbf{many distinct configurations}: a fine scan over $M$, $S$, and the mass-function width all land on Pantheon$+$ at $\chi^2/\mathrm{dof} \approx 0.44$--$0.45$ and far above the Einstein--de~Sitter null (Figure~\ref{fig:hires-multiplicity}). A Pantheon-matching vantage is thus a generic, not a fine-tuned, feature of the model. Figure~\ref{fig:hires-cloud} shows one such configuration evolved at high resolution.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\linewidth]{fig5_hero_multiplicity_66fb244.png}
    \caption{Best-observer distance modulus $\mu(z)$ for seven distinct high-resolution External-Node configurations (virialized lattice, wide node-mass function; $M = 200$--$400\,M_{obs}$, $S = 20$--$22$ Gpc, varied mass-function widths), each at $10^4$--$10^5$ particles, overlaid on Pantheon$+$SH0ES. All cluster on analytic $\Lambda$CDM (red dashed, $\chi^2/\mathrm{dof}=0.436$) at $\chi^2/\mathrm{dof}=0.442$--$0.449$, and all sit far above the Einstein--de~Sitter no-dark-energy null (green, $0.843$). Many configurations reproduce the data: the match is generic.}
\label{fig:hires-multiplicity}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\linewidth]{fig6_hero_100k_cloud_66fb244.png}
    \caption{The observable cloud ($7.5\times10^4$ tracer particles; $M=200\,M_{obs}$, $S=22$ Gpc, virialized lattice, $\sigma=6$; best-observer $\chi^2/\mathrm{dof}=0.449$) evolving from $t=2.9$ to $13.8$ Gyr, centred on the cloud and shown at a fixed common scale so the growth is visible (the rare runaway tail is omitted). The tidal field imprints the mild anisotropy and clustering that the central diagnostics smooth over.}
\label{fig:hires-cloud}
\end{figure}

\paragraph{Convergence and honest reading.} The $\approx 0.44$ best-observer fit is \textbf{numerically converged}: across a resolution ladder spanning $10^4$ to $10^5$ particles and $5\times10^3$ to $1.2\times10^4$ time-steps, the best-observer $\chi^2/\mathrm{dof}$ stays at $0.442$--$0.449$ (Figure~\ref{fig:hires-ladder}), so it is not an under-resolution artefact. The honest reading is unchanged in spirit from above: the model \emph{matches} $\Lambda$CDM (best vantage $\approx 0.44$ versus $0.436$) and \emph{decisively rejects} the no-dark-energy null, but it does not robustly \emph{beat} $\Lambda$CDM --- at these well-sampled vantages the fraction strictly below $\Lambda$CDM is essentially zero, and the fit remains degenerate in $M/S^3$. As before, the isotropic Hubble diagram cannot discriminate the model from $\Lambda$; the distinguishing prediction is the anisotropy (Sections~\ref{sec:dipole}--\ref{sec:dark-flow}).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.78\linewidth]{fig7_hero_chi2_ladder_66fb244.png}
    \caption{Best-observer (purple) and central (blue) $\chi^2/\mathrm{dof}$ for the seven configurations of Figure~\ref{fig:hires-multiplicity}, spanning $10^4$--$10^5$ particles and $5$--$12\times10^3$ steps. The best-observer fit is stable at $\approx 0.44$ across the whole resolution ladder, between $\Lambda$CDM (red dashed) and well below the Einstein--de~Sitter null (green).}
\label{fig:hires-ladder}
\end{figure}
...
Dark flow ($l \approx 290$-$298°$), the Axis of Evil ($l \approx 260°$), and the predicted $H_0$ dipole (Section~\ref{sec:dipole}) all point to the same general sky region—expected if all trace the nearest HMEA.
...
Compared directly against the real Pantheon$+$SH0ES supernova data (Section~\ref{sec:from-data}), the from-simulation expansion history fits with $\chi^2/\mathrm{dof} = 0.501$ for uniform nodes at the central observer, improving to $\approx 0.44$ --- statistically indistinguishable from $\Lambda$CDM ($0.437$) --- once the nodes carry a realistic wide mass function and we score the best-matching observer rather than assuming we sit at the centre (Section~\ref{sec:hires}). This $\approx 0.44$ match is reached by \emph{many} configurations and is numerically converged to $10^5$ particles. Either way the model sits with $\Lambda$CDM and far from the Einstein--de~Sitter no-dark-energy null ($0.844$, which the data reject). The model thus produces genuine \emph{effective} dark energy, though it does not outperform $\Lambda$CDM and remains degenerate in $M/S^3$. As the traceless nature of the external tidal tensor implies (Section~\ref{sec:traceless}), the isotropic Hubble diagram cannot discriminate the lattice; the model's distinguishing signal is the anisotropy it predicts.
...
In this model, all three are consequences of a single geometric structure---and, being shear/dipole signatures rather than isotropic ones, they are precisely where the model parts ways with a pure cosmological constant.

Future work: full GR formulation, quantitative CMB multipole predictions from the lattice, and directional coherence tests using Euclid and LSST data---the anisotropy channel that the isotropic supernova fit cannot probe.
...
The simulation code used to generate the results in this paper is permanently archived on Zenodo at \url{https://doi.org/10.5281/zenodo.20481859}.
...
\bibitem{Brout2022} Brout, D., et al. "The Pantheon+ Analysis: Cosmological Constraints." \textit{The Astrophysical Journal} 938.2: 110 (2022).
\end{thebibliography}
\end{document}