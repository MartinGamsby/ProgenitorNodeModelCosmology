...
Following our analytical estimates, we tested configurations with node masses $M_{ext} \approx (5-10) \times 10^{55}$ kg and grid spacings $S \approx 20-30$ Gpc. The simulation evolves 300 test particles over a 6 Gyr period (from cosmic time $t = 10.8$ Gyr to $t = 16.8$ Gyr), covering the era of observed cosmic acceleration. Initial conditions are set to match $\Lambda$CDM predictions at $t = 10.8$ Gyr.

\begin{figure}
    \centering
    \includegraphics[width=1.0\linewidth]{figure_simulation_results.png}
    \caption{Comparison of $\Lambda$CDM (blue solid) and External-Node model (red dashed) showing: (top left) scale factor evolution, (top right) Hubble parameter evolution with realistic present-day value $H_0 \approx 70$ km/s/Mpc, (bottom left) ratio of physical sizes demonstrating $\pm 1\%$ agreement throughout 6 Gyr, and (bottom right) physical universe size versus node distance showing both models exhibit gentle upward curvature characteristic of accelerated expansion.}
\end{figure}

\subsection{Optimal Configuration and Results}

Through systematic parameter exploration, we identified an optimal configuration that reproduces $\Lambda$CDM expansion with remarkable fidelity:

\begin{itemize}
    \item Node mass: $M_{ext} = 800 \times M_{obs} \approx 8.0 \times 10^{55}$ kg
    \item Grid spacing: $S = 24$ Gpc  
    \item Initial universe radius: $R_0 = 11.59$ Gpc (identical to $\Lambda$CDM)
    \item Effective dark energy: $\Omega_{\Lambda,\text{eff}} = 2.555$
\end{itemize}

The specific values of $M_{ext} = 800 \times M_{obs}$ and $S = 24$ Gpc emerge from our numerical exploration and should not be interpreted as fundamental predictions. Rather, they demonstrate that parameter combinations exist within physically reasonable ranges (node masses $\sim 10^{55}$ kg, separations $\sim 20-30$ Gpc) that reproduce observations. A complete theory would need to explain why the meta-structure adopts these scales—perhaps through its own evolutionary dynamics analogous to how galactic masses are set by cooling times and feedback processes.

\paragraph{Quantitative Agreement:}
The external-node model achieves \textbf{99.4\% agreement} with standard $\Lambda$CDM over the 6 Gyr simulation period:
\begin{itemize}
    \item $\Lambda$CDM final radius: 17.78 Gpc
    \item External-node final radius: 17.89 Gpc
    \item Relative error: 0.6\%
\end{itemize}

The simulated universe size remains within $\pm 1\%$ of $\Lambda$CDM predictions throughout the entire evolution, demonstrating stable long-term agreement without fine-tuning.
...
\begin{itemize}
    \item Late-time cosmic acceleration (the past $\sim$6 Gyr)
    \item Reproduction of $\Lambda$CDM expansion dynamics to 99.4\% accuracy
    \item A classical gravitational mechanism that mimics dark energy
