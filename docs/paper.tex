...
While this parameter provides a fit to observational data, it suffers from severe theoretical pathologies, most notably the fine-tuning problem \cite{Weinberg1989}—requiring a cancellation of vacuum energy terms to 120 decimal places—and the coincidence problem \cite{Steinhardt1999}.
...
By deriving a modified acceleration equation based on external tidal forces, we demonstrate that a grid of attractors with mass $M \approx 8.0 \times 10^{55}$ kg (800$\times$M$_{obs}$) at a spacing of $S = 22$ Gpc achieves a 99.85\% endpoint match to $\Lambda$CDM with R$^2$=0.9991 for size evolution (RMSE $\approx$ 0.006 Gpc) over an 8 Gyr period without invoking exotic fluids or modified gravity.
We validate this mechanism through N-body simulations covering the late-universe expansion era ($t=5.8 \rightarrow 13.8$ Gyr).
...
The irregular (virialized) nature of the meta-structure produces a dipole anisotropy in the Hubble constant of $\Delta H_0/H_0 \approx 4$-$9\%$, comparable to the observed Hubble Tension ($\sim$8.6\%) \cite{Riess2021}.
...
All three anomalies are predicted to share a common preferred direction---toward the nearest or most massive HMEA node---providing a unified geometric origin for observations that are otherwise treated as unrelated statistical flukes.
...
While our analytical estimate predicts $S \approx 39$ Gpc, systematic numerical exploration reveals optimal matches at $S = 20$-50 Gpc, with the best balance occurring at $S = 22$ Gpc.
This $\sim$77\% discrepancy likely arises from the analytical derivation's single-node assumption, whereas the simulation uses a 3$\times$3$\times$3 grid whose combined tidal field from 26 nodes is substantially stronger than single-node geometry.
The broader range of viable $S$ values (20-70 Gpc) demonstrates that the mechanism is robust rather than fine-tuned to the analytical target.
...
We begin at $t = 5.8$ Gyr rather than the Big Bang because this toy model explicitly focuses on late-time acceleration—early universe physics are outside our current scope (Section 5.2).
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\linewidth]{sim_plots_1f349b1_2000p_5.8-13.8Gyr_800.0M_1centerM_22.0S_300steps_123seed_0.0rnd_Autod.png}
    \caption{Comparison of $\Lambda$CDM (blue solid) and External-Node model with M=800$\times$M$_{obs}$, S=22 Gpc (red dashed) showing: (top left) scale factor evolution, (top right) Hubble parameter evolution with realistic present-day value $H_0 \approx 70$ km/s/Mpc, (bottom left) ratio of physical sizes demonstrating $\pm 1\%$ agreement throughout 8 Gyr, and (bottom right) physical universe size versus node distance showing both models exhibit gentle upward curvature characteristic of accelerated expansion.
This configuration achieves 99.85\% endpoint match with R$^2$=0.9991 for size evolution (RMSE $\approx$ 0.006 Gpc).}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.87\linewidth]{sim_plots_1f349b1_2000p_5.8-13.8Gyr_50000.0M_1centerM_50.0S_300steps_123seed_0.0rnd_Autod.png}
    \includegraphics[width=0.87\linewidth]{sim_plots_1f349b1_2000p_5.8-13.8Gyr_500.0M_1centerM_20.0S_300steps_123seed_0.0rnd_Autod.png}
    \caption{Additional parameter configurations demonstrating mechanism robustness: (top) M=50000$\times$M$_{obs}$, S=50 Gpc (R$^2$=0.999 size), and (bottom) M=500$\times$M$_{obs}$, S=20 Gpc (R$^2$=0.999 size).
Multiple solutions across a wide parameter range (M from 500 to 50000$\times$M$_{obs}$, S from 20 to 50 Gpc) all reproduce $\Lambda$CDM expansion dynamics with $\sim$100\% endpoint match.}
\end{figure}
...
\begin{tabular}{lccc}
\hline
\textbf{M} & \textbf{S [Gpc]} & \textbf{Endpoint Match} & \textbf{Size $R^2$} \\
\hline
$800 \times M_{obs}$ & 22 & 99.85\% & 0.9991 \\
$50000 \times M_{obs}$ & 50 & 99.76\% & 0.9990 \\
$500 \times M_{obs}$ & 20 & 99.71\% & 0.9989 \\
\hline
\end{tabular}
\end{center}
...
Conversely, close configurations ($S = 20$ Gpc) may predict near future hyper-acceleration as the universe approaches the node boundary, offering testable predictions for deep-time cosmology.

\paragraph{R² Metric and Statistical Rigor:}
We employ the coefficient of determination ($R^2$) and root-mean-square error (RMSE) to quantify agreement with $\Lambda$CDM:
\[
...
We compute $R^2$ for universe size evolution over the full 8 Gyr simulation period.
RMSE provides a complementary measure of absolute deviation in physical units.

\paragraph{Quantitative Agreement:}
Using the $M_{ext} = 800 \times M_{obs}$, $S = 22$ Gpc configuration as an example:
\begin{itemize}
    \item $\Lambda$CDM final size: 14.50 Gpc
    \item External-node final size: 14.52 Gpc
    \item Endpoint match: 99.85\%
    \item Size $R^2$: 0.9991 (RMSE $\approx$ 0.006 Gpc)
\end{itemize}

The model tracks $\Lambda$CDM expansion dynamics throughout the 8 Gyr evolution, achieving near-perfect agreement in both endpoint size and the full expansion trajectory.
...
\begin{itemize}
    ...
    \item Size $R^2$ (last 4 Gyr): 0.9269
\end{itemize}

The endpoint match appears respectable ($\sim$96\%), but the size $R^2$ reveals systematic deviation: the matter-only trajectory progressively diverges from $\Lambda$CDM during the acceleration era.

\paragraph{External-Node vs Matter-Only:}
Comparing external-nodes ($M_{ext} = 800 \times M_{obs}$, $S = 22$ Gpc) to matter-only:

\begin{center}
\begin{tabular}{lcc}
\hline
\textbf{Model} & \textbf{Endpoint Match} & \textbf{Size $R^2$ (last 4 Gyr)} \\
\hline
$\Lambda$CDM (baseline) & 100\% & 1.0000 \\
External-Node & 99.85\% & 0.9917 \\
Matter-only & 96.06\% & 0.9269 \\
\hline
\end{tabular}
\end{center}

The external-node model achieves near-perfect size $R^2$ (0.992), demonstrating it replicates the acceleration \textit{dynamics} throughout the trajectory, not just the coincidental endpoint.
Matter-only's significantly lower $R^2$ (0.927) confirms that deceleration alone produces a qualitatively different expansion curve.
...
The size $R^2$ serves as a discriminator for spurious matches: matter-only cannot replicate acceleration dynamics across 4 Gyr of late-time evolution, even if it reaches a reasonably similar endpoint.
...
\begin{enumerate}
    \item \textbf{Mechanism Validation}: External gravitational sources can produce accelerated expansion indistinguishable from $\Lambda$CDM at the $\sim$99.9\% level (R$^2$=0.9991) using only classical Newtonian gravity.
...
\item \textbf{Matter-Only Falsification}: The matter-only model's significantly lower size $R^2$ (0.927 vs 0.992 for external nodes in the late-time regime) demonstrates that deceleration cannot mimic acceleration dynamics, even if endpoints approximately align.
...
\begin{itemize}
    \item Late-time cosmic acceleration (8 Gyr period from $t = 5.8$ to $13.8$ Gyr)
    \item Reproduction of $\Lambda$CDM expansion dynamics to $> 99\%$ endpoint accuracy with $R^2 = 0.9991$ for size evolution
...
The remarkable agreement with $\Lambda$CDM (99.85\% endpoint match, $R^2$=0.9991 for size evolution) suggests this direction warrants further theoretical development, particularly in constructing the full relativistic framework and testing against additional observational constraints.
...
Extended simulations (20 Gyr, from $t = 5.8$ to 25.8 Gyr) with M = 800 $\times$ M$_{obs}$, S = 22 Gpc show:
...
For the M = 800, S = 22 Gpc configuration, the current universe (RMS radius $\sim$7 Gpc) has $R/S \approx 0.32$.
...
For M = 800 $\times$ M$_{obs}$, S = 22 Gpc, $R \approx 7.25$ Gpc (today):

\begin{itemize}
    \item Position irregularity only (5\%): $\Delta H_0 / H_0 \approx 2.1\%$ (grid average)
    \item Mass variation only (20\%): $\Delta H_0 / H_0 \approx 2.9\%$ (grid average)
    \item \textbf{Combined (position + mass)}: $\Delta H_0 / H_0 \approx 3.6\%$ ($\sim$2.5 km/s/Mpc, grid average)
    \item \textbf{Single nearest node (worst case)}: $\Delta H_0 / H_0 \approx 8.7\%$ ($\sim$6.1 km/s/Mpc)
    \item Observed Hubble Tension: $\sim$8.6\% (6 km/s/Mpc difference between 67 and 73 km/s/Mpc)
\end{itemize}

\paragraph{Implications:}
The predicted dipole ($\sim$3.6\% grid average, up to $\sim$8.7\% worst-case) is comparable to the Hubble Tension ($\sim$8.6\%).
This provides a falsifiable prediction with significant discriminating power: if high-precision all-sky $H_0$ measurements (e.g., from Euclid, LSST) detect a dipole anisotropy at the $\sim$4-9\% level aligned with no known local structure, it would strongly support the external-node hypothesis.
...
The predicted $H_0$ dipole of $\Delta H_0/H_0 \approx 3.6\%$ (Section 7.2) corresponds to an asymmetric acceleration that, integrated over cosmic time, produces bulk velocities of order:
\[
v_{bulk} \sim \frac{\Delta H_0}{H_0} \times H_0 \times R_{eff} \approx 0.036 \times 70 \text{ km/s/Mpc} \times 100 \text{ Mpc} \approx 250 \text{ km/s}
\]
This is consistent with the observed $\sim$400 km/s within a factor of two---reasonable given the simplified analytical estimate. The single nearest-node scenario ($\Delta H_0/H_0 \approx 8.7\%$) would yield $\sim$600 km/s, bracketing the observed value.
...
Through N-body simulations with 2000 particles, we have demonstrated that multiple parameter configurations (M = 500-50000 $\times$ M$_{obs}$, S = 20-70 Gpc) reproduce $\Lambda$CDM expansion dynamics with $\sim$100\% endpoint accuracy and R$^2$ = 0.9991 for size evolution (RMSE $\approx$ 0.006).
The primary configuration (M = 800 $\times$ M$_{obs}$ $\approx$ 8.0 $\times$ 10$^{55}$ kg, S = 22 Gpc) achieves 99.85\% endpoint match with R$^2$=0.9991 for size evolution over 8 Gyr ($t = 5.8 \rightarrow 13.8$ Gyr).
This validates the core mechanism: external gravitational sources can mimic dark energy using only classical physics.

Beyond the expansion match, the same lattice geometry---with no additional parameters---generates predictions consistent with three independent large-scale anomalies: the Hubble Tension ($\Delta H_0/H_0 \approx 4$-$9\%$), the CMB Axis of Evil (correlated quadrupole-octopole alignment), and dark flow ($\sim$250-600 km/s bulk motions).
