...
While this parameter fits observational data, it suffers from severe theoretical pathologies, most notably the fine-tuning problem \cite{Weinberg1989}—requiring a cancellation of vacuum energy terms across 120 orders of magnitude—and the coincidence problem \cite{Steinhardt1999}.
This paper presents a proof-of-concept toy model: a geometric framework termed the \textbf{External-Node Model}.
In this model, the observable universe is represented as a finite Friedmann-Robertson-Walker (FRW) region originating from the destabilization of a Progenitor Node within a larger, static lattice of Hyper-Massive External Attractors (HMEAs).

Cosmic acceleration arises naturally from the tidal gravitational influence of the surrounding lattice, as the bubble expands toward neighboring nodes, rather than from a repulsive "push" from the vacuum.
Through N-body simulations spanning the late-universe expansion era ($t=5.8 \rightarrow 13.8$ Gyr), we show that multiple lattice configurations across a wide mass range ($M = 92$-$9000 \times M_{obs}$, $S = 15$-$38$ Gpc) reproduce $\Lambda$CDM expansion dynamics with $R^2 > 0.99$ for size evolution and $R^2 > 0.95$ for expansion rate, without invoking exotic fluids or modified gravity.
Interestingly, the same lattice geometry—without additional free parameters—also produces patterns consistent with several observed large-scale anomalies: the Hubble Tension ($\Delta H_0/H_0 \approx 5$-$11\%$ \cite{Riess2022}), the CMB ``Axis of Evil'' \cite{Land2005}, and large-scale ``dark flow'' \cite{Watkins2023}, suggesting these features may be emergent consequences of the underlying geometry.

While this work does not yet address early-universe physics, it demonstrates that dark energy is not inevitable; classical gravitational structures can reproduce the late-time acceleration of the universe.
As a proof-of-concept, this model challenges the assumption that the Cosmological Principle holds at super-horizon scales.
\end{abstract}
...
Conceptually, this resembles a super-scaled cosmic web in virial equilibrium: mass concentrations are stable over vast timescales, balanced by mutual gravitation, but irregular in detail—much like the distribution of galaxies within a relaxed cluster.

These concentrations, termed \textbf{Hyper-Massive External Attractors (HMEAs)}, are compact objects far older than our universe's 13.8 Gyr, with masses exceeding the total mass of our observable universe ($M_{ext} \gg M_{obs}$) and separated by voids much larger than the observable universe.
...
This distance places the nodes well beyond the current extent of the observable universe.
Despite their enormous mass, HMEAs need not produce detectable electromagnetic signatures—as argued in Section~\ref{sec:defenses}, they are most naturally interpreted as ancient black holes that have long since exhausted their local fuel supply, rendering them gravitationally detectable but electromagnetically silent.
...
The mass range spans two orders of magnitude ($92$-$9000 \times M_{obs}$), demonstrating that the mechanism is not fine-tuned to a singular point in parameter space.
...
The remaining discrepancy may partly reflect the model's own predicted anisotropy.

\paragraph{Quantitative Agreement:}
Using the primary balanced-optimization configuration ($M_{ext} = 9000 \times M_{obs}$, $S = 38$ Gpc; Figure~\ref{fig:primary}), the model tracks $\Lambda$CDM to within 2\% in final size ($R^2_{size} = 0.9954$, $R^2_{rate} = 0.9630$).
Size-only optimization can push $R^2_{size}$ to 0.9991, at the cost of expansion rate fidelity as discussed above.
\paragraph{Hubble Parameter:}
The model reproduces realistic present-day values ($H_0 \approx 70$ km/s/Mpc) without requiring adjustment---the gravitational forces from the external grid naturally generate the observed magnitude of cosmic acceleration (Figure~\ref{fig:primary}).

\subsection{Matter-Only Comparison}
\label{sec:matter-only}

A matter-only cosmology (no dark energy, no external nodes) provides a critical baseline.
While matter-only reaches a respectable $\sim$96\% endpoint match, the expansion rate $R^2$ exposes the difference between coincidental endpoint agreement and genuine acceleration dynamics (Table~\ref{tab:comparison}).
...
The expansion rate $R^2$ is the key discriminator: external nodes achieve 0.96 versus matter-only's 0.84, confirming that the external-node mechanism provides genuine acceleration dynamics, not merely a fortuitous final size.
...
Several critical aspects of cosmology remain outside the current scope:
\begin{itemize}
...
\noindent A complete theory would require a full relativistic treatment of the meta-structure's metric, proper formulation of tidal forces in curved spacetime, and reconciliation with the Cosmological Principle's domain of validity.

We do not claim this model represents physical reality.
The value of this work lies in demonstrating that well-motivated classical alternatives to vacuum energy deserve investigation, and that the Cosmological Principle may not hold at super-horizon scales.
...
\label{sec:defenses}
An immediate objection is the invisibility of the proposed attractors: a mass of $\sim 10^{55}$ kg should emit detectable radiation.
The most parsimonious explanation is that HMEAs are ancient black holes that have long since exhausted their local fuel supply.
An HMEA is vastly older than the current expansion bubble ($t_{node} \gg 13.8 \text{ Gyr}$)—possibly quadrillions of years old.
...
Without an active accretion disk, a black hole is electromagnetically silent—detectable only through its gravitational gradient, not its luminosity.
Furthermore, Hawking radiation is negligible at these masses: $T_H \propto 1/M$ yields $T_H \sim 10^{-83}$ K for $M \sim 10^{55}$ kg—effectively zero at the source.
...
The HMEA framework implies a cyclical, evolutionary cosmology that reframes our place in the universe.
...
We are the fuel for the lattice of attractors.

\section{Conclusion}
\label{sec:conclusion}
The External-Node Model offers a geometric solution to the Dark Energy problem.
By reframing cosmic acceleration as the consequence of gravitational attraction from a larger, trans-observable manifold, we eliminate the need for fine-tuned vacuum energy and the associated theoretical crises of standard cosmology.

N-body simulations demonstrate that multiple parameter configurations across a wide mass range (M = 92-9000 $\times$ M$_{obs}$, S = 15-38 Gpc) reproduce $\Lambda$CDM expansion dynamics with $R^2 > 0.99$ for size evolution and $R^2 > 0.95$ for expansion rate over 8 Gyr ($t = 5.8 \rightarrow 13.8$ Gyr), validating the core mechanism using only classical physics.
...
These are typically treated as unrelated curiosities; in the External-Node Model, they are necessary consequences of a single geometric structure sharing a common preferred direction.
While each connection remains qualitative, the convergence of multiple independent lines of evidence from a parameter-free geometric prediction is suggestive.

Future work must develop the full general relativistic formulation, compute quantitative CMB multipole predictions from the lattice geometry, and test directional coherence between the predicted $H_0$ dipole, CMB multipole alignment, and bulk flow vector using upcoming survey data from Euclid and LSST.
If the predicted directional coherence is confirmed, it would establish that dark energy need not be an intrinsic property of spacetime, but rather a consequence of our universe's place within a larger gravitational structure.
