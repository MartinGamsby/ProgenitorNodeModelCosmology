...
By deriving a modified acceleration equation based on external tidal forces, we demonstrate through N-body simulations covering the late-universe expansion era ($t=5.8 \rightarrow 13.8$ Gyr, 8 Gyr) that multiple grid configurations across a wide mass range ($M = 92$-$9000 \times M_{obs}$, $S = 15$-$38$ Gpc) reproduce $\Lambda$CDM expansion dynamics with R$^2 > 0.99$ for size evolution and R$^2 > 0.95$ for expansion rate, without invoking exotic fluids or modified gravity.
Size-only optimization can achieve R$^2 = 0.9991$ for size, though at the cost of expansion rate fidelity---an expected tradeoff since the size curve is an integrated quantity that is less sensitive to the expansion rate profile.
...
While our analytical estimate predicts $S \approx 39$ Gpc for $M_{ext} \approx 500 \times M_{obs}$, systematic numerical exploration reveals optimal matches across a broad parameter space ($S = 15$-$70$ Gpc), with balanced-optimization configurations clustering at $S = 15$-$38$ Gpc.
...
% TODO: User will regenerate figures with new balanced-optimization configs
\begin{figure}[htbp]
    \centering
    ...
    \caption{Comparison of $\Lambda$CDM (blue solid) and External-Node model with M=9000$\times$M$_{obs}$, S=38 Gpc (red dashed) showing: (top left) scale factor evolution, (top right) Hubble parameter evolution with realistic present-day value $H_0 \approx 70$ km/s/Mpc, (bottom left) ratio of physical sizes, and (bottom right) physical universe size versus node distance showing both models exhibit gentle upward curvature characteristic of accelerated expansion.
...
Multiple solutions across a wide mass range (M from 92 to 9000$\times$M$_{obs}$, S from 15 to 38 Gpc) all reproduce $\Lambda$CDM expansion dynamics with R$^2 > 0.99$ for size and R$^2 > 0.95$ for expansion rate.}
\end{figure}
...
\textbf{Size-only optimization} maximizes the size curve R$^2$ alone, which can achieve higher size fidelity at the cost of expansion rate agreement (see Section 4.3).
...
The mass range spans over two orders of magnitude ($92$-$9000 \times M_{obs}$), yet all configurations reproduce $\Lambda$CDM dynamics with R$^2 > 0.99$ for size and R$^2 > 0.95$ for expansion rate—substantially exceeding the matter-only baseline (R$^2_{rate} = 0.84$).
Close configurations ($S = 15$ Gpc) may predict near future hyper-acceleration as the universe approaches the node boundary, offering testable predictions for deep-time cosmology.
...
A balanced optimization that weights both size and expansion rate identifies a family of configurations with R$^2_{size} > 0.99$ and R$^2_{rate} > 0.95$ across a wide mass range (Table 1).
...
We compare the isotropic RMS expansion rate of our N-body cloud to the Friedmann equation's $H(t)$, but the external-node model predicts anisotropic expansion (Section 7.2).
...
Using the balanced-optimization configuration $M_{ext} = 9000 \times M_{obs}$, $S = 38$ Gpc as the primary example:
\begin{itemize}
...
Size-only optimization can push the size R$^2$ to 0.9991 (endpoint 99.85\%), but this tradeoff is discussed in Section 4.3.
...
Comparing the balanced-optimization external-node configuration ($M_{ext} = 9000 \times M_{obs}$, $S = 38$ Gpc) to matter-only:
...
\begin{enumerate}
    \item \textbf{Mechanism Validation}: External gravitational sources can produce accelerated expansion matching $\Lambda$CDM with R$^2_{size} > 0.99$ and R$^2_{rate} > 0.95$ using only classical Newtonian gravity. Size-only optimization can push R$^2_{size}$ to 0.9991, demonstrating the mechanism's fidelity.
...
\begin{itemize}
    ...
    \item Reproduction of $\Lambda$CDM expansion dynamics with R$^2 > 0.99$ for size evolution and R$^2 > 0.95$ for expansion rate (balanced optimization), or R$^2 = 0.9991$ for size alone
...
The strong agreement with $\Lambda$CDM (R$^2 > 0.99$ for size, R$^2 > 0.95$ for expansion rate across multiple configurations) suggests this direction warrants further theoretical development, particularly in constructing the full relativistic framework and testing against additional observational constraints.
...
The predicted $H_0$ dipole of $\Delta H_0/H_0 \approx 4.6\%$ (Section 7.2) corresponds to an asymmetric acceleration that, integrated over cosmic time, produces bulk velocities of order:
\[
...
\]
This is consistent with the observed $\sim$400 km/s---reasonable given the simplified analytical estimate. The single nearest-node scenario ($\Delta H_0/H_0 \approx 11.3\%$) would yield $\sim$790 km/s, bracketing the observed value.
...
Size-only optimization can push the size R$^2$ to 0.9991, though at the cost of expansion rate fidelity---a tradeoff inherent to optimizing an integrated versus derivative quantity (Section 4.3).
