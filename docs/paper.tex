...
Cosmic acceleration arises naturally from the tidal gravitational influence of the surrounding lattice, as the bubble expands toward the effective capture radius of neighboring nodes, rather than from a repulsive "push" from the vacuum.
...
These concentrations, termed \textbf{Hyper-Massive External Attractors (HMEAs)}, are compact objects far older than our universe's 13.8 Gyr, with masses exceeding the total mass of our observable universe ($M_{ext} \gg M_{obs}$) and separated by voids ($S \gg R_{H}$).
...
Close configurations ($S = 15$ Gpc) may predict hyper-acceleration as the universe approaches the node boundary (Section~\ref{sec:phantom}).
...
Future work must develop the full general relativistic formulation, compute quantitative CMB multipole predictions from the lattice geometry, and test directional coherence between the predicted $H_0$ dipole, CMB multipole alignment, and bulk flow vector using upcoming survey data (Euclid, LSST).
The convergence of geometric predictions with multiple independent observational anomalies suggests that classical alternatives to dark energy deserve serious theoretical investigation.
