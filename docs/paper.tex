...
We present a toy model, the \textbf{External-Node Model}, in which the observable universe is a finite FRW region expanding within a lattice of Hyper-Massive External Attractors (HMEAs).
Cosmic acceleration comes from tidal gravity: as the bubble expands toward neighboring nodes, it is pulled apart—no exotic physics required.
N-body simulations over the late-universe era ($t=5.8 \rightarrow 13.8$ Gyr) show that configurations spanning $M = 92$-$9000 \times M_{obs}$, $S = 15$-$38$ Gpc reproduce $\Lambda$CDM expansion with $R^2 > 0.95$ for expansion rate and $R^2 > 0.99$ for size.

The same lattice geometry, with no additional parameters, also produces patterns consistent with three large-scale anomalies: the Hubble Tension ($\Delta H_0/H_0 \approx 5$-$11\%$ \cite{Riess2022}), the CMB ``Axis of Evil'' \cite{Land2005}, and dark flow \cite{Watkins2023}.
This work does not address early-universe physics, but it demonstrates that classical gravity can reproduce late-time acceleration without dark energy.
\end{abstract}
...
The accelerating expansion of the universe \cite{Riess1998, Perlmutter1999} is conventionally explained by the Cosmological Constant ($\Lambda$)—a constant vacuum energy density.
...
The \textbf{Coincidence Problem} \cite{Steinhardt1999}: dark energy and matter densities happen to be comparable ($\Omega_\Lambda \sim \Omega_m$) in the present epoch—a coincidence with no explanation.

We propose a different interpretation.
Rather than assuming the FRW metric describes all of spacetime, we model the observable universe as a finite FRW region expanding within a larger space containing discrete massive objects.
...
A companion \textbf{Progenitor Hypothesis} (Section~\ref{sec:progenitor}) addresses isotropy: the Big Bang originated from a destabilized node within the meta-structure, inheriting the symmetry of its equilibrium position.
...
Our framework is different: a single lattice geometry, motivated by the need to reproduce cosmic acceleration, simultaneously predicts patterns consistent with the Hubble Tension, the CMB ``Axis of Evil,'' and dark flow (Section~\ref{sec:predictions})—with no additional parameters.
...
We propose it holds only within our local bubble.
Beyond our horizon, we model a \textbf{Virialized Meta-Structure}: a gravitationally balanced arrangement of mass concentrations, stable over vast timescales but irregular in detail—analogous to galaxies within a relaxed cluster.
...
\begin{enumerate}
\item \textbf{Origin}: Our universe originates from a \textbf{Progenitor Node}—a former member of the Virialized Grid, sitting at a gravitational equilibrium point between its neighbors.
\item \textbf{Event}: The node destabilized (critical mass accumulation, quantum instability, or collision) and transitioned from a bound object to a rapidly expanding cloud of matter and radiation.
\item \textbf{Isotropy}: Because the Progenitor sat at an equilibrium point, expansion proceeds symmetrically into the potential wells of the surrounding structure—isotropy without fine-tuning.
\end{enumerate}

\section{Dynamics and Quantitative Analysis}
\label{sec:dynamics}
Expansion is governed by two competing forces: internal self-gravity (deceleration) and external tidal gravity (acceleration).

\subsection{The Tidal Acceleration Mechanism}
\label{sec:tidal}
In standard Newtonian cosmology, a shell of matter decelerates due to the enclosed mass $M_{int}$—a consequence of the shell theorem, which requires a continuous spherical external distribution.
Our model violates this condition: the external masses are discrete points, not a smooth shell.

The resulting tidal force increases with distance from the center.
At the exact center ($R=0$), forces from the surrounding grid cancel.
As a test galaxy moves outward, it climbs the potential gradient of the nearest HMEA.

The tidal acceleration from an external mass $M_{ext}$ at distance $S$:
...
This derivation uses only the nearest node.
In a virialized structure, forces cancel at the equilibrium point ($R = 0$); as the bubble expands, tidal contributions scale as $\sim GM/d^3$, so the nearest face node at distance $S$ dominates over edge nodes at $S\sqrt{2}$ ($\sim 35\%$ contribution) and corner nodes at $S\sqrt{3}$ ($\sim 19\%$).
The N-body simulation (Section~\ref{sec:numerical}) uses all 26 nodes; the single-node calculation here is an order-of-magnitude estimate.

The linear $R$ dependence is the key result.
In the Friedmann equations, the cosmological constant produces an acceleration $H_0^2 \Omega_\Lambda R$—also linear in $R$.
A uniform external tidal field is mathematically indistinguishable from $\Lambda$ in the non-relativistic limit.
...
Numerical exploration finds viable configurations across $S = 15$-$70$ Gpc, with balanced-optimization results clustering at $S = 15$-$38$ Gpc—a broad range, not a fine-tuned point.
...
A 3$\times$3$\times$3 cubic lattice with 26 external nodes, our universe at the center (no node).
HMEAs sit at integer multiples of the grid spacing $S$.
A real meta-structure would have irregular spacing and varying masses; the symmetric grid is a simplification for testing viability.
Distant nodes contribute negligibly to the tidal force, so 3$\times$3$\times$3 is sufficient.
...
Quality checks: matter-only must never exceed $\Lambda$CDM at any timestep, center-of-mass drift is monitored, and runaway particles are flagged.

\paragraph{Numerical Robustness:}
We report the worst-performing random seed for each configuration.
...
Above this threshold, expansion dynamics are insensitive to particle count.
...
No single optimum exists—a family of solutions spans two orders of magnitude in mass.
...
Size $R^2$ measures how well the simulated radius tracks $\Lambda$CDM—an integrated, smooth quantity.
Expansion rate $R^2$ measures agreement in $H(t)$—a derivative, harder to match.

Size-only optimization (e.g., M=800$\times$M$_{obs}$, S=22 Gpc, $R^2_{size} = 0.9991$) can achieve near-perfect size fits, but the underlying $H(t)$ follows a different path that happens to integrate to the same displacement.
Balanced optimization gives $R^2_{size} > 0.99$ and $R^2_{rate} > 0.95$; matter-only achieves only $R^2_{rate} \approx 0.84$.

The expansion rate comparison is approximate: we compare isotropic RMS expansion to the Friedmann $H(t)$, but the model predicts anisotropic expansion (Section~\ref{sec:dipole}).
The remaining discrepancy may partly reflect this predicted anisotropy.
The Hubble tension ($\sim$8.6\%) also means the ``correct'' $H(t)$ is itself uncertain.

\paragraph{Primary Results:}
The primary configuration ($M_{ext} = 9000 \times M_{obs}$, $S = 38$ Gpc; Figure~\ref{fig:primary}) tracks $\Lambda$CDM to within 2\% in final size ($R^2_{size} = 0.9954$, $R^2_{rate} = 0.9630$) and reproduces $H_0 \approx 70$ km/s/Mpc without adjustment.

\subsection{Matter-Only Comparison}
\label{sec:matter-only}

Matter-only (no dark energy, no external nodes) reaches $\sim$96\% endpoint match, but expansion rate $R^2$ reveals the difference (Table~\ref{tab:comparison}).
...
Expansion rate $R^2$ is the discriminator: 0.96 (external nodes) vs. 0.84 (matter-only). External nodes produce genuine acceleration, not merely a coincidental endpoint.
...
Classical alternatives to vacuum energy are viable and worth investigating.

\section{Predictions and Falsifiability}
\label{sec:predictions}
The model makes three distinct predictions—plus one qualitative consistency—all from the same lattice geometry with no additional parameters.
...
The same geometry, with no additional parameters, produces patterns consistent with three anomalies usually treated as unrelated: the Hubble Tension ($\Delta H_0/H_0 \approx 5$-$11\%$), the CMB Axis of Evil, and dark flow.
