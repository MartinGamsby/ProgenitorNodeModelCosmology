...
class RK4Integrator(Integrator):
    ...
    Higher accuracy but not symplectic (energy drift)
    """
    
    def step(self, dt):
        """
        Take one RK4 timestep
        ...
        """
        # Save initial state
        pos0 = self.particles.get_positions().copy()
        vel0 = self.particles.get_velocities().copy()
        
        # k1
        a1 = self.calculate_total_forces()
        v1 = vel0
        
        # k2
        self.particles.set_accelerations(a1)
        for i, p in enumerate(self.particles.particles):
            p.pos = pos0[i] + 0.5 * dt * v1[i]
            p.vel = vel0[i] + 0.5 * dt * a1[i]
        a2 = self.calculate_total_forces()
        v2 = vel0 + 0.5 * dt * a1
        
        # k3
        self.particles.set_accelerations(a2)
        for i, p in enumerate(self.particles.particles):
            p.pos = pos0[i] + 0.5 * dt * v2[i]
            p.vel = vel0[i] + 0.5 * dt * a2[i]
        a3 = self.calculate_total_forces()
        v3 = vel0 + 0.5 * dt * a2
        
        # k4
        self.particles.set_accelerations(a3)
        for i, p in enumerate(self.particles.particles):
            p.pos = pos0[i] + dt * v3[i]
            p.vel = vel0[i] + dt * a3[i]
        a4 = self.calculate_total_forces()
        v4 = vel0 + dt * a3
        
        # Final update
        for i, p in enumerate(self.particles.particles):
            p.pos = pos0[i] + (dt / 6.0) * (v1[i] + 2*v2[i] + 2*v3[i] + v4[i])
            p.vel = vel0[i] + (dt / 6.0) * (a1[i] + 2*a2[i] + 2*a3[i] + a4[i])
            p.acc = a4[i]
        
        self.particles.time += dt
    
    def evolve(self, t_end, n_steps, save_interval=10):
        """Evolve system (same interface as Leapfrog)"""
        dt = (t_end - self.particles.time) / n_steps
        
        snapshots = []
        
        print(f"Running RK4 integration...")
        print(f"  dt = {dt:.2e} s ({dt/(365.25*24*3600*1e6):.2f} Myr)")
